...
export const Jean = () => {
    ...
    const [jean, setJean] = useState<SneakerType | null>(null)
    
    const {jeans} = useFetchJeans();

    useLayoutEffect(() => {
        jeans.filter((jean: SneakerType) => {
            if (jean.id === path) {
                setJean(jean)
                return jean
            }
        ...
        <div className='flex'>
            <div className='flex flex-wrap w-[50%]'>
                {jean?.images.map((img, i) => {
                    return <img key={i} width="400" src={img} alt={jean.brand} />
                })}
            </div>
            <div className='flex flex-col w-[50%] pr-[20%]'>
                <div className='justify-end self-end'>
                    <div className='justify-end self-end'>
                        <h1 className='text-6xl'>{jean?.brand}</h1>
                        <h2 className='text-3xl mt-6'>{jean?.model}</h2>
                    </div>
                    <div className='mt-8 self-end'>
                        <div>
                            Colors:
                            {
                                jean?.colors.map((color, i) => {
                        ...
                        <div className='mt-6 flex'>
                            Sizes:
                            {
                                jean?.sizes.map((size, i) => {
                                    return <span key={i} className="border-2 px-2 py-1 mx-1">{size}</span>
                                })
                            }
                        </div>
                    </div>
                    <h2 className='mt-6 text-3xl'>Price: {jean?.price}$</h2>
